...
router.get('/stats', (req, res) => {
  ...
  const vip = (db.prepare('SELECT COUNT(*) AS c FROM users WHERE is_vip = 1').get())?.c || 0
  const admins = (db.prepare('SELECT COUNT(*) AS c FROM users WHERE is_admin = 1').get())?.c || 0
  const orders = (db.prepare('SELECT COUNT(*) AS c FROM orders').get())?.c || 0
  const revenue = (db.prepare("SELECT COALESCE(SUM(amount),0) AS c FROM orders WHERE status='paid'").get())?.c || 0
  const aiCalls = (db.prepare('SELECT COUNT(*) AS c FROM ai_logs').get())?.c || 0
...
router.post('/users/:id/remove-vip', (req, res) => {
  req.db.prepare('UPDATE users SET is_vip=0, vip_expire_at=NULL, updated_at=? WHERE id=?').run(Date.now(), +req.params.id)
  res.json({ ok: true })
})
