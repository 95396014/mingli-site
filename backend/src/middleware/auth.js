...
async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    if (user.free_daily_date !== today) {
      await db.prepare('UPDATE users SET free_daily_used = 0, free_daily_date = ? WHERE id = ?').run(today, user.id)
      user.free_daily_used = 0
      user.free_daily_date = today
    }
    if (user.is_vip && user.vip_expire_at && user.vip_expire_at < Date.now()) {
      await db.prepare('UPDATE users SET is_vip = 0, vip_expire_at = NULL WHERE id = ?').run(user.id)
      user.is_vip = 0
    }
    req.user = user
    next()
